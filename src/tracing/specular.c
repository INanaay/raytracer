...
** Last update Mon May 22 16:24:11 2017 anatole zeyen
*/
			       ...
			       sfVector3f *normal_normal)
{
  ...
  angle = normal_light.x * normal_normal.x + normal_light.y *
    normal_normal.y + normal_light.z * normal_normal.z;
  reflect_vector.x = -2 * angle * (normal_light.x + normal_normal.x)
    + normal_light.x;
  reflect_vector.y = -2 * angle * (normal_light.y * normal_normal.y)
    + normal_light.y;
  reflect_vector.z = -2 * angle * (normal_light.z * normal_normal.z)
    + normal_light.z;
  return (reflect_vector);
}

float		specular(sfVector3f dir_vector, sfVector3f *light,
			 float coef, sfVector3f *normal) // normal = la normale du pt d'intersection entre light_v et obj
{
  float		angle;
